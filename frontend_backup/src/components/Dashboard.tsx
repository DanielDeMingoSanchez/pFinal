  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 transition-colors duration-300 centrado-principal">
      ...
      <motion.div 
        ...
        className="container mx-auto px- py-6" >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {resultadosBusqueda.map((doc) => (
          <div key={doc.id} className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md">
            ...
            <p className="text-gray-600 dark:text-gray-400">{doc.descripcion.length > 20 ? doc.descripcion.substring(0, 20) + '...' : doc.descripcion}</p>
      ...
      <p className="text-gray-500 dark:text-gray-400 mt-2 text-center">
          ...
          <div className="flex justify-center w-full mb-12">
            {!mostrarFormulario ? (
              <motion.button
                ...
                className="mx-auto bg-blue-600 text-white rounded-lg px-5 py-2.5 shadow-sm hover:bg-blue-700 transition-colors flex items-center justify-center"
                whileHover={{
                  scale: 1.02,
        ...
        >
          ...
          {documentosRecientes.length > 0 ? (
            <div className="carousel-container bg-white dark:bg-gray-800 shadow-lg">
              {/* Contenedor con animación de ciclo infinito */}
              <div className="carousel-cards animate-scroll-infinite">
                    ...
                    <div className="p-6 h-full flex flex-col">
                      <div className="flex-grow">
                        <h3 
                          ...
                          title={doc.titulo}
                        >
                          {doc.titulo.length > 10 ? doc.titulo.substring(0, 10) + '...' : doc.titulo}
                        </h3>
                        <p 
                          className="text-sm text-gray-600 dark:text-gray-300 mb-4"
                          title={doc.descripcion}
                        >
                          {doc.descripcion.length > 20 ? doc.descripcion.substring(0, 20) + '...' : doc.descripcion}
                        </p>
                        
                        <div className="flex items-center justify-between text-sm text-gray-500 dark:text-gray-400 mb-4">
                          <div className="flex items-center">
                            <span className="truncate max-w-[120px]" title={doc.usuario?.nombre || 'Usuario desconocido'}>
                              <FaUser className="inline-block mr-1" /> {doc.usuario?.nombre || 'Usuario desconocido'}
                            </span>
                          </div>
                          <div className="text-right">
                            <FaCalendar className="inline-block mr-1" /> {doc.fechaCreacion ? new Date(doc.fechaCreacion).toLocaleDateString() : 'Fecha desconocida'}
                          </div>
                        </div>
                        
                        <div className="flex items-center space-x-2 mb-4">
                          <span className="flex items-center text-xs">
                            <FaEye className="mr-1 text-gray-500" /> {doc.vistas || 0}
                          </span>
                          <span className="flex items-center text-xs">
                            <FaDownload className="mr-1 text-gray-500" /> {doc.descargas || 0}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-2 mt-auto justify-center">
                        <button 
                          className="btn-blue px-3 py-1.5 text-sm flex items-center"
                          onClick={() => abrirDocumento(doc)}
                        >
                          <FaEye className="mr-1 text-xs" /> Ver
                        </button>
                        ...
                        {doc.usuario.id === auth.currentUser?.uid && (
                          <button 
                            className="btn-blue px-3 py-1.5 text-sm flex items-center"
                            onClick={() => borrarDocumento(doc)}
                          >
                            <FaTrash className="mr-1 text-xs" /> Borrar
                          </button>
                        )}
                      </div>
                    </div>
                ...
                {/* Duplicamos exactamente los mismos documentos para crear el ciclo continuo */}
                {documentosRecientes.map((doc) => (
                  <div 
              ...
              {/* Instrucción sobre la funcionalidad */}
              <div className="absolute bottom-2 right-4 text-xs text-gray-500 dark:text-gray-400">
                <span className="opacity-70"></span>
              </div>
            </div>
          ) : (
            <div className="text-center text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 rounded-lg shadow-sm p-8 dark:border dark:border-gray-700">
              <p className="mb-4">No hay documentos recientes disponibles</p>
              <button
                onClick={() => cargarDatos()}
