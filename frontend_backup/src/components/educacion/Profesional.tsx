...
const Profesional: React.FC = () => {
  ...
  useEffect(() => {
    const obtenerDocumentos = async () => {
      try {
        setCargando(true);
        const q = query(collection(db, "documentos"), where("categoria", "==", "profesional"));
  ...
  return (
    <EducationPageWrapper
      title="Profesional"
      description="Documentos y recursos para el ámbito profesional y laboral. Materiales de formación continua y desarrollo de habilidades profesionales."
    ...
    >
      {cargando ? (
        ...
        <Box textAlign="center" my={6} p={4} bgcolor="rgba(255,255,255,0.8)" borderRadius={2}>
          <FaBriefcase size={48} color="#EC4899" />
          <Typography variant="h6" mt={2} mb={1}>
            No hay documentos profesionales
          </Typography>
          <Typography color="text.secondary" paragraph>
            Sé el primero en compartir recursos para el ámbito profesional y laboral.
          </Typography>
          <Button 
            variant="contained" 
            color="primary"
